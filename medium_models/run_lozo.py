...
class DynamicTrainingArguments(TrainingArguments):
    ...
    momentum: float = field(
        default=0.9,
        metadata={"help": "momentum for SGD"}
    )
    nesterov: bool = field(
        default=False,
        metadata={"help": "whether use nesterov"}
    )
    # =======================================================
    ...
    kfac: bool = field(
        default=False,
        metadata={"help": "whether use kfac-lozo"}
    )
    # =============================================================
...
def main():
    ...
    run_name = f"{model_name}_{data_args.task_name}_"
    if training_args.kfac:
        run_name += "kfac_"
    if training_args.lowrank_zo:
        run_name += "lozo_"
    run_name += f"{training_args.optimizer}_lr{training_args.learning_rate:.0e}_bs{training_args.per_device_train_batch_size}_"
    if training_args.lowrank_zo:
        run_name += f"r{training_args.rank}_nu{training_args.step_interval}"
    
    wandb.login(key="726be770e2a351a53a5aab7e7f7772dfc603a233")
    wandb.init(project="kfac-lozo-medium", name=run_name, config=training_args)
